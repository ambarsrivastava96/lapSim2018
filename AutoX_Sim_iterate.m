...
for i = 1:(length(v_corner_max) - 1)
    ...
    if (competition.trackData(i,2) == 0) % Find Straights
        if (i == 1) % Check if launch/start straight
            [Accel_time, v_final] = func_iter_Accel_time(car, competition.trackData(i,3), v_launch, competition.trackData(i,1));
        else
            [Accel_time, v_final] = func_iter_Accel_time(car, competition.trackData(i,3), v_corner_max(i - 1), competition.trackData(i,1));
        end
        ...
        if (i == (length(v_corner_max) - 1))
            ...
            [Accel_time, v_accel, energy] = func_iter_Accel_time(car, competition.trackData(i,3), v_corner_max(i - 1), competition.trackData(i,1));
        ...
        elseif ((i ~= 1) && (v_corner_max(i - 1) > v_corner_max(i + 1)))
            ...
            [Braking_dist, Braking_time, energyRecovered] = func_iter_Braking_dist(car, competition.trackData(i,3), v_corner_max(i - 1), v_corner_max(i + 1));
        ...
        elseif (v_final < v_corner_max(i + 1))
            ...
            while v_check<v_corner_max(i+1)
                distReq = distReq + 1;
                [Accel_time,v_check,energy] = func_iter_Accel_time(car,competition.trackData(i,3),v_final,distReq);
...
%             Accel_time2 = 0;
            ...
            if (i == 1)
                [Accel_time, v_final, energy] = func_iter_Accel_time(car, competition.trackData(i,3), v_launch, competition.trackData(i,1));
            else
                [Accel_time, v_final, energy] = func_iter_Accel_time(car, competition.trackData(i,3), v_corner_max(i - 1), competition.trackData(i,1));
            end
            ...
            if (i == 1)
                [Accel_time, v_accel, energy] = func_iter_Accel_time(car, competition.trackData(i,3), v_launch, competition.trackData(i,1));
            else
                [Accel_time, v_accel, energy] = func_iter_Accel_time(car, competition.trackData(i,3), v_corner_max(i - 1), competition.trackData(i,1));
            end
            
            [Braking_dist, Braking_time, energyRecovered] = func_iter_Braking_dist(car, competition.trackData(i,3), v_accel, v_corner_max(i + 1));
            ...
            while (abs((Braking_dist + Accel_dist - competition.trackData(i,1)))/competition.trackData(i,1) > 0.01)
                ...
                if (i == 1)
                    [Accel_time, v_accel, energy] = func_iter_Accel_time(car, competition.trackData(i,3), v_launch, Accel_dist);
                else
                    [Accel_time, v_accel, energy] = func_iter_Accel_time(car, competition.trackData(i,3), v_corner_max(i - 1), Accel_dist);
                end
